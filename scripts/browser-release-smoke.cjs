...
(async () => {
  ...
  try {
    for (const width of [1280, 390]) {
      ...
      assert.ok((await download.innerText()).includes('เปิดอ่านเอกสารเก่าได้ โดยไม่ต้องยกเลิก'));
      assert.ok((await download.innerText()).includes('JSON ไม่มีไฟล์แนบอยู่ข้างใน'));
      assert.ok((await page.locator('#download-confirm-btn').getAttribute('href')).endsWith('BillNgai-2.0.4-universal.dmg'));
      await page.screenshot({ path: path.join(out, `download-${width}.png`) });
      await download.locator('aside > p').last().scrollIntoViewIfNeeded();
      assert.ok(await page.locator('#download-confirm-btn').isVisible());
      assert.ok(await download.getByRole('link', { name: 'ดาวน์โหลดไฟล์เดียวกันจาก GitHub' }).isVisible());
      await page.screenshot({ path: path.join(out, `download-bottom-${width}.png`) });
      ...
      assert.ok((await page.locator('body').innerText()).includes('Mac Direct 2.0.4'));
